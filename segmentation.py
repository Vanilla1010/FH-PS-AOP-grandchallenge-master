...
class MySegmentation(object):
    ...
    #   初始化UNET
    #---------------------------------------------------#
    def __init__(self, path_model=None, **kwargs):
        self.__dict__.update(self._defaults)
        for name, value in kwargs.items():
            setattr(self, name, value)

        self.model = unet(num_classes = self.num_classes, backbone=self.backbone)
    ...
    # 加载模型，作用与原来的generate函数相同
    def load_model(self, onnx=False):
    ...
    def process_image(self, input_image):
        ...
        # 输入图片需要为PIL.Image格式
        image = Image.fromarray(input_image[0])
        # Putting images into the network
        output = self.get_miou_png(image)
